...
function StaticPageContent({ page }: { page: StaticPage }) {
  if (page === "about") {
    return (
      <article className="static-content">
        ...
        <h2>記録と共有</h2>
        <p>普段の編集データはお使いのブラウザ（ローカル）に保存されます。共有リンクを作成した場合のみ、短縮リンク（/s/ID）を提供するためにグラフデータが本サービスのデータベースへ保存され、リンクを知っている人に閲覧専用のグラフを共有できます。</p>
      </article>
    );
  }
  ...
  if (page === "privacy-policy") {
    return (
      <article className="static-content">
        ...
        <h1>プライバシーポリシー</h1>
        <p>My Life Chart（以下「本サービス」）は、以下の方針で利用者の情報を取り扱います。</p>
        <h2>ブラウザ内に保存される情報</h2>
        <p>入力したグラフ、出来事、表示設定、テーマ設定は、原則として利用中のブラウザのローカルストレージに保存されます。また、短縮リンクを作成したブラウザには、共有ID、グラフタイトル、作成日時および削除用トークンが保存されます。ブラウザのデータを削除すると、これらの情報や削除権限を失う場合があります。</p>
        <h2>共有時に保存される情報と利用目的</h2>
        <p>「共有」機能で短縮リンクを作成した場合、グラフタイトル、出来事、日付、値および表示設定を含むグラフデータを、共有ページの提供に必要な範囲で本サービスのデータベースへ保存します。併せて、推測困難な共有ID、削除用トークンのハッシュ値、作成日時を保存します。削除用トークンそのものやIPアドレスは同データベースへ保存しません。</p>
        <h2>共有リンクの公開範囲</h2>
        <p>共有ページは編集不可で、検索エンジンに登録しない設定を行っていますが、URLを知っている人は誰でも閲覧できます。ランダムな共有IDは内容の秘密性を保証するものではありません。個人情報、第三者の情報、その他公開したくない情報の入力・共有には十分ご注意ください。</p>
        <h2>外部サービスと不正利用対策</h2>
        <p>本サービスは、配信、データ保存、アクセス制御および不正利用対策にCloudflareのサービスを利用しています。また、自動投稿などを防ぐためCloudflare Turnstileを使用しています。共有リンクの作成時には、IPアドレス、ブラウザや端末に関する情報、認証結果など、セキュリティ機能の提供に必要な通信情報がCloudflareによって処理されます。Turnstileへグラフの入力内容を認証情報として送信することはありません。</p>
        <h2>保存期間と削除</h2>
        <p>共有データには現在、自動的な有効期限を設定していません。削除用トークンを保持するブラウザから削除されるまで、またはサービスの安全な運営、規約違反への対応、容量管理その他運営上必要な理由により運営者が削除するまで保存されます。削除用トークンを失った場合、画面から削除できなくなることがあります。</p>
        <h2>アクセス解析・広告</h2>
        <p>本サービス独自のアクセス解析サービスや広告配信サービスは導入していません。ただし、Cloudflareによるセキュリティ、運用およびサービス提供上のログや統計情報の処理が行われる場合があります。</p>
        <h2>お問い合わせ</h2>
        <p>本ポリシーや共有データに関するお問い合わせは、<a href="https://github.com/FukuyaMan/my-life-chart/issues">GitHub Issues</a>からご連絡ください。本人確認や対象データの特定ができない場合、ご要望に対応できないことがあります。</p>
        <h2>改定日</h2>
        <p>2026年8月31日 改定</p>
      </article>
    );
  ...
  return (
    <article className="static-content">
      ...
      <h2>共有リンクの提供と制限</h2>
      <p>短縮URLで共有したデータは、URLを知る人が閲覧できます。共有する内容と共有先は利用者の責任で確認してください。本サービスの共有機能は無料枠の範囲で運用されており、アクセス集中、利用枠超過、保守その他の事情により、リンクの作成や閲覧が一時的に制限される場合があります。</p>
      <h2>データの管理</h2>
      <p>ブラウザのデータ削除や端末変更により、編集データ、共有リンクの履歴、削除用トークンが失われる場合があります。必要に応じてJSONエクスポートやPNG保存機能でバックアップを行ってください。共有データに保存期間の保証はなく、運営上必要な場合には予告なく削除されることがあります。</p>
      <h2>投稿内容と権利</h2>
      <p>利用者は、入力・共有する内容について必要な権利を有し、第三者の権利やプライバシーを侵害しないことを確認するものとします。入力内容の権利は利用者に帰属し、本サービスは共有機能を提供するために必要な範囲で当該データを保存・配信します。</p>
      <h2>免責事項</h2>
        ...
        <div className="share-options">
          <button onClick={handleCreateShare} disabled={isCreating}>
            ...
            <span>
              ...
              <small>共有データは短縮リンクを提供するため本サービス上に保存されます</small>
            </span>
          </button>
